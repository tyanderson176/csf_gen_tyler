...
def dets_from_configs(configs, desired_S, desired_parity, desired_L_z=[0]):
	"""
	...
	for det in all_dets:
		...
		if (tot_s_z, parity) == (desired_S, desired_parity) and tot_l_z in desired_L_z:
...
def get_csfs_by_configs(N, desired_L, desired_S, desired_parity, configs_str, config_targets=[], config_tol=1, desired_L_z=[0]):
	"""
	Generate all CSFs in a set of configurations with desiredorbital angular momentum, spin 
	and parity.
	...
	for i, config in enumerate(configs):
		...
		l_z, l_plus, l_minus, s_z, s_plus, s_minus = build_operators(max_n)	
		...
		if (l_min <= desired_L <= l_max) and (s_min <= desired_S <= s_max):
			...
			all_dets, filtered_dets = dets_from_configs(config, desired_S, desired_parity, desired_L_z=desired_L_z)
			...
			for j, det in enumerate(filtered_dets):
				...
				projected_state = project_ang_mom(projected_state, s_z, s_plus, s_minus, wrong_spin_vals)
...
if __name__ == '__main__':
	...
	wrong_ang_mom_vals = []
	#state = Determinant(
	#projected_state = project_ang_mom(state, l_z, l_plus, l_minus, wrong_ang_mom_vals)
	configs_str = ['1P2_2P2_3P2']
	get_csfs_by_configs(N, desired_L, desired_S, desired_parity, configs_str, desired_L_z=desired_L_z)
